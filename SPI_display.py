...
spi = busio.SPI(clock=board.IO10, MOSI=board.IO9) #don't change pins for now
tft_cs = board.IO8 #can change pin
tft_dc = board.IO18 #can change pin
tft_reset = board.IO17 #can change pin
...
pwm = pwmio.PWMOut(board.IO16, duty_cycle=2**16 - 1)
...
def overwrite_text(text, scale, x, y, color_hex, index):
    #parameters:    text - string of what you want displayed
    #               scale - scale of text. try 2 or 3
    #               x - x coordinate of text to be displayed, from 0 to 320
    #               y - y coordinate of text to be displayed, from 0 to 240
